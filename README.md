...
>│    ├─ TC-01_.spec.ts        ← отдельный файл на каждый тест-кейс.                        
>│    └─...                                  
>│                                                   
>├─ server/                    ← Веб-интерфейс управления тестами                                    
...
- Веб-интерфейс Test UI
   ...
   - **Кнопка открытия HTML отчета Playwright в соседней вкладке** → `/report/index.html` (официальный Playwright)
   - **После завершения тестирования - открытие Детального отчета** → парсинг `playwright-report.json` (TC-01, TC-02... статус/время)  
   - **Таблица История запусков** → `test-history.json` (дата + результат + Подробно)
...
- Покрытие автоматизации
     ...
     - Автоматизированы тест-кейсы, покрывающие:
         ...
         - скорость генерации при максимальном объеме данных (в идеале нужны вводные)
